  ...
  try {
    ...
    try {
      ...
      if (t && to) {
        ...
        `;
        const fromHdr = (process.env.MAIL_FROM && process.env.MAIL_FROM.trim()) || (process.env.SMTP_USER ? `Garage Gators <${process.env.SMTP_USER}>`  : `Garage Gators <no-reply@garagegators.com>`);
...
app.post('/api/util/test-mail', (req, res) => {
  try {
    ...
    const fromHdr = (process.env.MAIL_FROM && process.env.MAIL_FROM.trim()) || (process.env.SMTP_USER ? `Garage Gators <${process.env.SMTP_USER}>`  : `Garage Gators <no-reply@garagegators.com>`);
    return t.sendMail({ from: fromHdr, to, subject: `[Test] Garage Gators SMTP test @ ${now}` , html: `<p>This is a test email sent at ${now}.</p>` })
